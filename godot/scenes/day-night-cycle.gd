...
var time:float:
	...
	set(value):
		Global.world_time = value
var past_minute:int= -1


func _ready() -> void:
	if Global.world_time < 0:
		Global.world_time = INGAME_TO_REAL_MINUTE_DURATION * MINUTES_PER_HOUR * INITIAL_HOUR
	else:
		time = Global.world_time


func _process(delta: float) -> void:
	time += delta * INGAME_TO_REAL_MINUTE_DURATION * INGAME_SPEED
	Global.world_time = time
